...
const generateTokens = (user) => {
	...
	const exp = jwt.verify(accessToken, ACCESS_TOKEN_SECRET, {
...
app.post("/express-api/auth/refresh", (req, res) => {
	...
	try {
		...
		return res
			.status(401)
			.send({ message: "Refresh token verification failed." });
	}

	if (refreshTokenPayload.accessToken !== accessToken) {
		return res.status(401).send({
			message: "This refresh token is not connected to provided access token.",
		});
	}
...
app.get("/express-api/protected-route", requireAuth, (req, res) => {
	return res
		.status(200)
		.send({ data: "Some secret data only for authenticated users." });
});
